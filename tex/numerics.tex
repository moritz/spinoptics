...
of the waves, and in the SO regime that is not known a priori --
rather it's an intermediate
result of the numerical calculation\footnote{but one we're not really
interested in, and by using the Fisher-Lee relation we shortcut the path and
never explicitly calculate it}.

So since one can't match the bases between SO region and right lead, one has
...
cosine form that the tight binding model implies. This explains why there's a
